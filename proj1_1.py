...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 22 16:50:49 2018
...
#Use Max Min from the window to do linear-scaling over the whole picture
...
for j in range(rows):
    for i in range(cols):
        ...
        if t > 0.008856:
            ...
            L = 903.3 * t
        #truncate L to min and max of the window
        L = min(tempH, L)
        L = max(tempL, L)
        #Linear-Scaling to get a new L
        nL = int((L - tempL) * (ranH - ranL) / (tempH - tempL) + ranL +0.5) 
...
print("Winsow Lumincae range:",tempL,tempH)
